...
// 媒合分數計算
function calcScore(me: { industries: string[]; regions: string[] }, p: Profile): number {
  ...
  const levelBonus = p.contact_level === "senior" ? 40 : p.contact_level === "middle" ? 20 : 10;
  ...
  useEffect(() => {
    async function load() {
      ...
      const scored = data
        ...
        .map((p: Profile) => ({
          ...p,
          score: calcScore({ industries: myIndustries, regions: myRegions }, p),
        }))
            ...
            <div key={p.id} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
              ...
              <div className="flex flex-wrap gap-1 mb-3">
                {p.industries.slice(0, 3).map((ind) => (
                  <span key={ind} className="text-xs bg-purple-100 text-purple-900 px-2 py-1 rounded">{ind}</span>
                ))}
                {p.regions.slice(0, 2).map((reg) => (
                  <span key={reg} className="text-xs bg-purple-50 text-purple-600 px-2 py-1 rounded border border-purple-100">{reg}</span>
                ))}
              </div>
              ...
              {revealedId === p.id ? (
                <div className="bg-green-50 border border-green-200 rounded-xl p-3 flex items-center justify-between">
                  ...
                  <button
                    onClick={() => navigator.clipboard.writeText(p.line_id || "")}
